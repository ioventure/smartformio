...
export class MonthInput extends FormInput {
  ...
  private _validate(): void {
    ...
    if (!this._isValidMonthFormat(monthValue)) {
      this.setErrorMessage(
        fieldName,
        'Please enter a valid month in the format YYYY-MM.'
      );
      return;
    }
    ...
    if (this.options.min && monthDate < new Date(this.options.min + '-01')) {
      this.setErrorMessage(
        fieldName,
        `Month cannot be earlier than ${this.options.min}.`
      );
    }
    ...
    if (this.options.max && monthDate > new Date(this.options.max + '-01')) {
      this.setErrorMessage(
        fieldName,
        `Month cannot be later than ${this.options.max}.`
      );
    }
    ...
    if (
      ...
      !this.options.customValidation(monthValue)
    ) {
