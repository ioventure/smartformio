...
export class DateTimeInput extends FormInput {
  ...
  private _validate(): void {
    ...
    if (
      this.options.min &&
      new Date(dateTimeValue) < new Date(this.options.min)
    ) {
      this.setErrorMessage(
        fieldName,
        `Date and time cannot be earlier than ${this.options.min}.`
      );
    }
    ...
    if (
      this.options.max &&
      new Date(dateTimeValue) > new Date(this.options.max)
    ) {
      this.setErrorMessage(
        fieldName,
        `Date and time cannot be later than ${this.options.max}.`
      );
    }
    ...
    if (
      ...
      !this.options.customValidation(dateTimeValue)
    ) {
