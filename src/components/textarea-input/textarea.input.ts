...
export class TextareaInput extends FormInput {
  ...
  private _validate(): void {
    ...
    if (
      ...
      this.setErrorMessage(
        fieldName,
        `Minimum length should be ${this.options.minLength}.`
      );
    }
    ...
    if (
      ...
      this.setErrorMessage(
        fieldName,
        `Maximum length should be ${this.options.maxLength}.`
      );
    }
    ...
    if (this.options.maxWords) {
      ...
      if (wordCount > this.options.maxWords) {
        this.setErrorMessage(
          fieldName,
          `Maximum word count is ${this.options.maxWords}.`
        );
      }
    }
    ...
    if (this.options.minWords) {
      ...
      if (wordCount < this.options.minWords) {
        this.setErrorMessage(
          fieldName,
          `Minimum word count is ${this.options.minWords}.`
        );
      }
    }
  }
